/*
 * Homebrew AES adaptation of tiny-aes-c for creating test vectors for each step.
 * Note that security isn't important here but it should still produce results that
 * are reproducable with OpenSSL.
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>

#include "self_aes.h"

static const uint8_t sbox [256] = {
    ,
};


static const uint8_t rcon[11] = { 0x8D, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

void keyexpansion(aes_ctx * restrict ctx, const uint8_t key[static restrict 16]) {
    size_t i, j, k;
    uint8_t tmp[4], tmp2;

    for (i = 0; i < 4; i++) {
        (ctx->roundkey)[(i*4)+0] = key[(i*4)+0];
        (ctx->roundkey)[(i*4)+1] = key[(i*4)+1];
        (ctx->roundkey)[(i*4)+2] = key[(i*4)+2];
        (ctx->roundkey)[(i*4)+3] = key[(i*4)+3];
    }

    for (i = 4; i < 44; i++) {
        k = (i-1)*4;
        tmp[0] = (ctx->roundkey)[k+0];
        tmp[1] = (ctx->roundkey)[k+1];
        tmp[2] = (ctx->roundkey)[k+2];
        tmp[3] = (ctx->roundkey)[k+3];

        if (i%4 == 0) {
            tmp2 = tmp[0];
            tmp[0] = tmp[1];
            tmp[1] = tmp[2];
            tmp[2] = tmp[3];
            tmp[3] = tmp2;

            tmp[0] = sbox[tmp[0]];
            tmp[1] = sbox[tmp[1]];
            tmp[2] = sbox[tmp[2]];
            tmp[3] = sbox[tmp[3]];

            tmp[0] ^= rcon[i/4];
        }
 
        j = i*4;
        k=(i-4)*4;

        (ctx->roundkey)[j + 0] = (ctx->roundkey)[k + 0] ^ tmp[0];
        (ctx->roundkey)[j + 1] = (ctx->roundkey)[k + 1] ^ tmp[1];
        (ctx->roundkey)[j + 2] = (ctx->roundkey)[k + 2] ^ tmp[2];
        (ctx->roundkey)[j + 3] = (ctx->roundkey)[k + 3] ^ tmp[3];
    }
}

void subbytes(aes_ctx * restrict ctx) {
    uint8_t i, j;

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            (ctx->state)[j][i] = sbox[(ctx->state)[j][i]];
        }
    }
}

void shiftrows(aes_ctx * restrict ctx) {
    uint8_t tmp;

    /* 1st row 1 col to left. */
    tmp = (ctx->state)[0][1];
    (ctx->state)[0][1] = (ctx->state)[1][1];
    (ctx->state)[1][1] = (ctx->state)[2][1];
    (ctx->state)[2][1] = (ctx->state)[3][1];
    (ctx->state)[3][1] = tmp;

    /* Rotate 2nd row 2 col to left. */
    tmp = (ctx->state)[0][2];
    (ctx->state)[0][2] = (ctx->state)[2][2];
    (ctx->state)[2][2] = tmp;
    tmp = (ctx->state)[1][2];
    (ctx->state)[1][2] = (ctx->state)[3][2];
    (ctx->state)[3][2] = tmp;

    /* Rotate 3rd row 3 col to left. */
    tmp = (ctx->state)[0][3];
    (ctx->state)[0][3] = (ctx->state)[3][3];
    (ctx->state)[3][3] = (ctx->state)[2][3];
    (ctx->state)[2][3] = (ctx->state)[1][3];
    (ctx->state)[1][3] = tmp;
}

void mixcolumns(aes_ctx * restrict ctx) {
    uint8_t tmp1, tmp2, tmp3;
    size_t i;

    for (i = 0; i < 4; i++) {
        tmp1 = (ctx->state)[i][0];
        tmp2 = (ctx->state)[i][0] ^ (ctx->state)[i][1] ^ (ctx->state)[i][2] ^ (ctx->state)[i][3];

        tmp3 = (ctx->state)[i][0] ^ (ctx->state)[i][1];
        tmp3 = ((tmp3<<1) ^ (((tmp3>>7) & 1) * 0x1B));
        (ctx->state)[i][0] ^= tmp2 ^ tmp3;

        tmp3 = (ctx->state)[i][1] ^ (ctx->state)[i][2];
        tmp3 = ((tmp3<<1) ^ (((tmp3>>7) & 1) * 0x1B));
        (ctx->state)[i][1] ^= tmp2 ^ tmp3;

        tmp3 = (ctx->state)[i][2] ^ (ctx->state)[i][3];
        tmp3 = ((tmp3<<1) ^ (((tmp3>>7) & 1) * 0x1B));
        (ctx->state)[i][2] ^= tmp2 ^ tmp3;

        tmp3 = (ctx->state)[i][3] ^ tmp1;
        tmp3 = ((tmp3<<1) ^ (((tmp3>>7) & 1) * 0x1b));
        (ctx->state)[i][3] ^= tmp2 ^ tmp3;
    }
}

void addroundkey(aes_ctx * restrict ctx, size_t round_num) {
    size_t i, j;

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            (ctx->state)[i][j] ^= (ctx->roundkey)[(round_num*16)+(i*4)+j];
        }
    }
}

void self_aes_test_ctx_gen(aes_ctx * restrict ctx, const uint8_t key[static restrict 16]) {
    size_t i, j;

    keyexpansion(ctx, key);

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            (ctx->state)[i][j] = 4*i+j;
        }
    }
}

void self_aes_encrypt(uint8_t out[static restrict 16],
                      const uint8_t plain[static restrict 16],
                      const uint8_t key[static restrict 16]) {
    aes_ctx ctx;
    size_t i, j;

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            ctx.state[i][j] = plain[4*i+j];
        }
    }

    keyexpansion(&ctx, key);
    addroundkey(&ctx, 0);

    for (i = 1; i < 10; i++) {
        subbytes(&ctx);
        shiftrows(&ctx);
        mixcolumns(&ctx);
        addroundkey(&ctx, i);
    }

    subbytes(&ctx);
    shiftrows(&ctx);
    addroundkey(&ctx, 10);

    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            out[4*i+j] = ctx.state[i][j];
        }
    }
}
