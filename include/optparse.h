/*
 * This is a slimmed down version of optparse by Chris Wellons.
 * Because its long options are used only once in the main file,
...
static void optparse_init(struct optparse *options, char **argv);

static int optparse(struct optparse *options, const char *optstring);

static int optparse_long(struct optparse *options,
...
static int optparse_long(struct optparse *options, const struct optparse_long *longopts, int *longindex) {
    int i;
    char *option = options->argv[options->optind];
    if (option == 0) {
        return -1;
